...
@section('page-subtitle', 'Update user information and profile')
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-10 mx-auto fade-up d1">
            <div class="card-box">

                @if (auth()->user()->role === 'admin')
                    <div class="info-note">
                        <i class="bi bi-info-circle-fill me-2"></i>
                        You are editing <strong>{{ $user->name }}</strong>'s profile. Changes will be saved to their
                        account.
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('dashboard.clients.update', $user->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-md-12">
                                <label class="form-label-dash">Select CV Design</label>
                                <select name="cv_template" class="form-control-dash">
                                    <option value="modern"
                                        {{ ($user->cv_template ?? 'modern') == 'modern' ? 'selected' : '' }}>Modern - Clean
                                        and professional</option>
                                    <option value="minimal"
                                        {{ ($user->cv_template ?? '') == 'minimal' ? 'selected' : '' }}>Minimal - Simple
                                        and elegant</option>
                                    <option value="creative"
                                        {{ ($user->cv_template ?? '') == 'creative' ? 'selected' : '' }}>Creative - Bold
                                        and colorful</option>
                                    <option value="professional"
                                        {{ ($user->cv_template ?? '') == 'professional' ? 'selected' : '' }}>Professional -
                                        Classic layout</option>
                                    <option value="sidebar"
                                        {{ ($user->cv_template ?? '') == 'sidebar' ? 'selected' : '' }}>Sidebar - Two
                                        column layout</option>
                                </select>
                                <small class="text-muted-custom" style="font-size: 11px; color: #7a869a;">Choose the
                                    design for this user's public CV</small>
                                @error('cv_template')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                    ...
                    <div class="d-flex justify-content-end gap-3 mt-4">
                        <a href="{{ route('dashboard.clients.show', $user->id) }}" class="btn-light-dash">Cancel</a>
